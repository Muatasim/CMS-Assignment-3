		...
		foreach ($notices as $notice) {
			...
			if( ( $logged_in_only && is_user_logged_in() ) || !$logged_in_only) {			
				if(pippin_check_notice_is_read($notice->ID, $user_ID) != true) { ?>
					<div id="attention-area" class="<?php echo strtolower(get_post_meta($notice->ID, '_notice_color', true)); ?> hidden">
						<a class="remove-notice" href="#" id="remove-notice" rel="<?php echo $notice->ID; ?>"><?php _e('X', 'notificaton'); ?></a>
